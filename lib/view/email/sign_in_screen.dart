...
class _SignInScreenState extends State<SignInScreen> {
                ...
                Expanded(
                  child: SingleChildScrollView(
                    child: Column(
                      ...
                      children: [
                        Center(
                          child: Image.asset(myLoading.isDark ? icLogo : icLogoLight, height: 100, width: 100),
                        ),
                        SizedBox(height: 20),
                        Center(
                          child: Text(
                            '${LKey.signInFor.tr} $appName',
                            style: TextStyle(fontSize: 18, fontFamily: FontRes.fNSfUiSemiBold, color: ColorRes.colorTextLight),
                          ),
                        ),
                        SizedBox(height: 30),
                        _title(title: LKey.enterYourMail.tr),
                        const SizedBox(height: 10),
                        LoginTextFiled(focusNode: emailFocus, controller: emailController, obscureText: false, keyboardType: TextInputType.emailAddress, isDarkMode: myLoading.isDark),
                        const SizedBox(height: 20),
                        _title(title: LKey.password.tr),
                        const SizedBox(height: 10),
                        LoginTextFiled(focusNode: passwordFocus, controller: passwordController, obscureText: true, keyboardType: TextInputType.visiblePassword, isDarkMode: myLoading.isDark),
                        const SizedBox(height: 20),
                        Row(
                            ...
                            InkWell(
                              ...
                              child: RichText(
                                text: TextSpan(
                                  ...
                                  style: TextStyle(color: Colors.grey, fontSize: 12, fontFamily: FontRes.fNSfUiLight),
                                  children: <TextSpan>[
                                    TextSpan(text: ' ${LKey.signUp.tr}', style: TextStyle(fontFamily: FontRes.fNSfUiBold, color: ColorRes.colorPink, fontSize: 12)),
                                  ],
                                ),
                              ),
                            ),
  ...
  Widget _title({required String title, String fontFamily = FontRes.fNSfUiMedium, double fontSize = 15}) {
    return Text(
      title,
      style: TextStyle(
  ...
  void onLoginBtnClick(MyLoading myLoading) async {
    ...
    signIn(email: emailController.text.trim(), password: passwordController.text.trim()).then((value) async {
      if (value == null) return;
      if (value.user?.email == emailController.text.trim()) {
        //if (value.user?.emailVerified == true) {
        sessionManager.saveString(KeyRes.password, passwordController.text.trim());
        ...
        params[UrlRes.userName] = value.user?.email != null ? value.user?.email!.split('@')[0] : value.user?.uid;
  ...
  Future<void> resetPassword() async {
    ...
    try {
      await _auth.sendPasswordResetEmail(email: resetPassController.text.trim());
