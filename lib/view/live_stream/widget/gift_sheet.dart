...
class GiftSheet extends StatelessWidget {
  final VoidCallback onAddNexusTap;
  final User? user;
  final Function(Gifts? gifts) onGiftSend;
  final SettingData? settingData;

  const GiftSheet({Key? key, required this.onAddNexusTap, this.user, required this.onGiftSend, required this.settingData}) : super(key: key);
                    ...
                    return InkWell(
                      onTap: () {
                        if (gift!.coinPrice! < user!.data!.myWallet!) {
                          ...
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(
                              content: Text('Insufficient Coins..! Please purchase nexuss'),
                      ...
                      child: Container(
                        ...
                        foregroundDecoration: BoxDecoration(
                            color: (gift?.coinPrice ?? 0) > (user?.data?.myWallet ?? 0) ? ColorRes.colorPrimaryDark.withOpacity(0.7) : Colors.transparent, borderRadius: BorderRadius.circular(10)),
                        decoration: BoxDecoration(
