...
class LiveStreamScreenViewModel extends BaseViewModel {
  ...
  void goLiveTap(BuildContext context) async {
    ...
    await ApiService().generateAgoraToken(registrationUser?.data?.identity).then((value) async {
      ...
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (c) => BroadCastScreen(registrationUser: registrationUser, agoraToken: value.token, channelName: registrationUser?.data?.identity),
        ),
      );
    });
  }
  ...
  void onImageTap(BuildContext context, LiveStreamUser user) async {
    ...
    //String authToken = user.agoraToken ?? "";
    CommonUI.showLoader(context);

    ApiService().agoraListStreamingCheck(user.hostIdentity ?? '', authToken, '${settingData?.agoraAppId}').then((value) {
          ...
          builder: (c) {
            return LiveStreamEndSheet(
              ...
              onExitBtn: () async {
                ...
                var collection = db.collection(FirebaseRes.liveStreamUser).doc(user.hostIdentity).collection(FirebaseRes.comment);
